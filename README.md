...
电商 AI 视频生成工作流：知识库驱动的提示词引擎 + 开放模型接入 + 批量生成。

用一句话说：**把「参考图 → 分镜 → 中文提示词 → 批量生成视频」整条流水线做成一个可安装的 Python 包**，提示词由知识库按镜精准注入，模型侧开放接入、不绑定任何单一厂商。

> 当前版本：v1.5.0（src 布局，`pip install` 可装；Python ≥ 3.9）

## 特性

- **开放模型接入，custom 零代码是主路径**：任意 OpenAI 兼容接口（自建网关、聚合平台等），只改 `.env` 即可接入视频/生图/LLM 三条链路；非兼容厂商（签名制）写一个 30–50 行的 provider 类注册即用（详见 `docs/PROVIDERS.md`）
- **知识库驱动，14 个电商品类**：品类档案（profiles）+ 场景光线 + 别名 + 合规红线等 35 个 JSON 文件程序化校验（`kbcheck` 35/35）
- **每镜精准注入，不堆提示词**：retriever 按镜检索词源（材质/精确/别名/tags 四级匹配），每镜只注入本镜所需素材，而不是把 4000–6000 字全量塞给模型
- **全中文提示词**：提示词三层规则（L1 锚定 / L2 动态 / L3 材质）+ 七要素，输出严格全中文
- **批量生成 + 能力校验拦截**：按所选模型在 `knowledge/models.json` 注册的能力参数（参考图数量/时长/分辨率/生图模式）在生成前拦截超限任务
- **合规红线拦截**：医疗/药品/保健等无资质品类流程拦截不接（`knowledge/compliance.json`）
- **测试基线**：78 个主套件用例 + 4 个开放接入集成用例（本地 mock server 端到端），全绿
